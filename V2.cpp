...
void printTabela(const vector<Mensagem>& mensagens, int rank) {
    printf("Tabela: Configuração para o aeroporto a%d.\n", rank + 1);
    printf("+---------+---------+--------------------+---------------+\n");
    printf("| Código  | Origem  | Horário de Chegada | Tempo de Voo  |\n");
    printf("+---------+---------+--------------------+---------------+\n");
    
    for (const auto& msg : mensagens) {
        if (msg.voo.is_pouso) {
            printf("| %7.3d | %7d | %18d | %12d |\n",
                   msg.voo.codigo,
                   msg.voo.origem,
                   msg.voo.horario,
                   msg.voo.tempo_voo);
    ...
    printf("+---------+---------+--------------------+---------------+\n");
    printf("| Código  | Destino | Horário de Partida | Tempo de Voo  |\n");
    printf("+---------+---------+--------------------+---------------+\n");
    for (const auto& msg : mensagens) {
        if (!msg.voo.is_pouso) {
            printf("| %7.3d | %7d | %18d | %12d |\n",
